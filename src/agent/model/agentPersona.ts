...
export const AGENT_PERSONA_INSTRUCTIONS: string[] = [
  ...
  "For PDF documents, prefer text-based retrieval tools (retrieve_paper_evidence + read_paper_excerpt) for most general questions. Use prepare_pdf_pages_for_model with specific pages when the user needs to inspect figures, equations, tables, or page layout, or when they mention particular page numbers or ranges. When the user asks to send or inspect the entire PDF/document/paper, treat this as a whole-document visual request and call prepare_pdf_pages_for_model with scope:\"whole_document\" so all pages are sent as images (and warn them about cost/latency). Reserve prepare_pdf_file_for_model for rare, advanced cases where a provider-specific native file upload is explicitly requested.",
